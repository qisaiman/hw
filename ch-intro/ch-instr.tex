...
In this work, magnetron sputtering (Denton Vacuum Desk IV) was used to coat thin layers of metals onto cleaned substrates. As shown in Fig.~\ref{fig:ch2magsp}, the metal target is the cathode and the specimen is anode. A dc voltage is applied to ionizes the argon gas, and a permanent magnet behind the target focuses the gas plasma onto the target and also deflect electrons from the sample. These ions are then accelerated to bombard the target foils, knocking out active atoms which subsequently deposit onto the substrates. 
\begin{figure}[htb]
\centering
\includegraphics[width=0.7\textwidth]{sputter.jpg}
